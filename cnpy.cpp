...
void cnpy::npz_save_fp16(std::string zipname, std::string fname, void* data, const std::vector<size_t>& shape, std::string mode) {
    int dsize=2;
    //first, append a .npy to the fname
    fname += ".npy";
...
//    create_special_npy_header(const std::vector<size_t>& shape, std::string endian, std::string dtype, std::string dsize)
    std::vector<char> npy_header = create_special_npy_header(shape, std::string(1,BigEndianTest()), "f", "2");

    size_t nels = std::accumulate(shape.begin(),shape.end(),1,std::multiplies<size_t>());
    size_t nbytes = nels*dsize + npy_header.size();

    //get the CRC of the data to be added
    uint32_t crc = crc32(0L,(uint8_t*)&npy_header[0],npy_header.size());
    if(nels>0) {
        crc = crc32(crc,(uint8_t*)data,nels*dsize);
    }
...
std::vector<char> cnpy::create_special_npy_header(const std::vector<size_t>& shape, std::string endian, std::string dtype, std::string dsize) {
    std::string dict_str;
    dict_str += "{'descr': '";
    dict_str += endian;
    dict_str += dtype;
